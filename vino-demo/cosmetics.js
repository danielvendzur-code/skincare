(() => {
  ...
  document.title = `${brand.name} – výber vína`;
  ...
  const state = {
    ...
    messages:[{role:'assistant',text:'Dobrý deň. Napíšte, aké víno máte radi alebo k čomu ho hľadáte. Pomôžem vám vybrať z našej ponuky.'}]
  };
  ...
  const INCLUDED = [
    ...
    ['Odpovedá aj o polnoci', 'Odrody, chuť, párovanie s jedlom aj porovnanie dvoch vín.'],
    ['Výber cez štyri otázky', 'Farba, chuť, počet fliaš a jedlo — na konci konkrétne víno.'],
  ...
  /* Headline and lead are the same on every page; the coffee pages read the
     same words. */
  const HEADING = 'Poradí zákazníkovi víno a odpovie mu na otázky.';
  const LEAD = 'Chatbot odpovie zákazníkovi na otázky o vašich produktoch. Cez štyri otázky ' +
    'mu vyberie víno podľa chuti a jedla. Predáte aj vtedy, keď pri tom nie ste.';
  ...
  const KEEPS = [
    ...
    ['Výber cez štyri otázky', 'na konci konkrétne víno z ponuky'],
    ['História konverzácií', 'vidíte, na čo sa zákazníci pýtajú']
  ];

  const CHIPS = ['Suché biele k rybe', 'Červené k mäsu', 'Niečo sladšie', 'Víno na darček'];
  ...
  root.innerHTML = `
    ...
    <div class="cx-launcher" id="cx-launcher">
      <button class="cx-teaser" id="cx-teaser" type="button"><b>Pomôcť s výberom vína?</b><span>4 otázky · konkrétne víno</span></button>
    ...
    <section class="cx-widget" id="cx-widget" role="dialog" aria-modal="true" aria-label="Poradca pri výbere vína ${esc(brand.name)}" aria-hidden="true">
      ...
      <nav class="cx-mode" aria-label="Režim poradcu">
        ...
        <button type="button" data-mode="advisor" aria-pressed="false">${icons.spark}<b>Výber vína</b></button>
    ...
    stage.innerHTML = `
      <section class="cx-chat">
        ...
        <div class="cx-chat-bottom">
          ...
          <form class="cx-composer" id="cx-form"><input id="cx-input" maxlength="500" autocomplete="off" placeholder="Opýtajte sa na víno…" aria-label="Otázka"><button type="submit" aria-label="Odoslať" ${state.busy?'disabled':''}>${icons.send}</button></form>
        </div>
      </section>`;
  ...
  function localReply(text) {
    ...
    const find = (...tags) => brand.products.find((p) => tags.every((t) => p.tags.includes(t)));
    const has = (re) => re.test(q);
    const color = has(/červen|cerven/) ? 'red' : has(/ružov|ruzov|rosé|rose/) ? 'rose' : has(/šumiv|sumiv|sekt|pét|pet-nat|perliv/) ? 'sparkling' : has(/biel/) ? 'white' : '';
    const style = has(/sladk|sladš|dezert/) ? 'sweet' : has(/such|svie|kysel/) ? 'crisp' : has(/pln|výrazn|vyrazn|sud|barrique/) ? 'bold' : has(/ovocn|jemn/) ? 'fruity' : '';
    const food = has(/ryb|hydin|kur|šalát|salat/) ? 'fish' : has(/mäs|mas|steak|gril|divin/) ? 'meat' : has(/syr|koláč|kolac/) ? 'cheese' : '';
    if (has(/darč|darc/)) {
      const gift = find('target') || find('bold') || brand.products[0];
      return `Na darček by som siahol po ${gift.name} (${gift.price}). ${gift.reason} Vo Výbere vína ešte zohľadníme, či obdarovaný pije skôr biele alebo červené.`;
    }
    if (color || style || food) {
      const wanted = [color, style, food].filter(Boolean);
      const hit = find(...wanted) || (color && style && find(color, style)) || (color && food && find(color, food)) || find(wanted[0]) || brand.products[0];
      return `Z ponuky ${brand.name} by som odporučil ${hit.name} (${hit.price}). ${hit.reason} Vo Výbere vína cez štyri krátke otázky nájdem aj ďalšie vína, ktoré sa k tomu hodia.`;
    }
    if (/odrod|ročník|rocnik|pôvod|povod|vinic|trat/.test(q)) {
      const first = brand.products[0];
      return `Odrodu a ročník nájdete pri každom víne na webe ${brand.name}. Napríklad ${first.name}: ${first.reason} Napíšte, aké víno máte radi, a vyberiem konkrétne.`;
    }
    if (/cena|ceny|cenu|koľko|kolko|stoj|draho|lacn/.test(q)) {
      const first = brand.products[0];
      return `Ceny sú pri každom víne — ${first.name} stojí ${first.price}. Cez štyri krátke kroky vo Výbere vína vyberiem to, ktoré vám sadne, aj s cenou.`;
    }
    return `Z ponuky ${brand.name} je dobrý začiatok ${brand.products[0].name}. Napíšte, aké víno máte radi alebo k akému jedlu, alebo prejdite štyri krátke kroky vo Výbere — vyberiem konkrétne víno aj s dôvodom.`;
  }
  ...
  function score(product) {
    const weights={skin:14,goal:10,routine:0,texture:6};
  ...
  const ANSWER_LABELS = {
    white:'biele', red:'červené', rose:'ružové', sparkling:'šumivé',
    crisp:'suché a svieže', fruity:'ovocné', bold:'plné', sweet:'sladšie',
    target:'na darček',
    fish:'k rybe a hydine', meat:'k mäsu', cheese:'k syrom a dezertom'
  };
    ...
    stage.innerHTML=`
      <section class="cx-result">
        ...
        <div class="cx-result-body">
          <span class="cx-kicker">Vaše víno</span>
          ...
          <section class="cx-why"><small>Prečo práve toto víno</small><p>${esc(product.reason)}</p></section>
          ...
          <p class="cx-result-note">Výber je orientačný podľa preferencií. Predaj alkoholu len osobám starším ako 18 rokov.</p>
  ...
  function resetAll(){state.step=0;state.answers={};state.result=null;state.alternative=null;state.interacted=false;state.busy=false;state.transitioning=false;state.messages=[{role:'assistant',text:'Dobrý deň. Napíšte, aké víno máte radi alebo k čomu ho hľadáte. Pomôžem vám vybrať z našej ponuky.'}];setMode('chat');}
