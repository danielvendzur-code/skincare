...
const [slug, out, base = 'http://127.0.0.1:8791'] = process.argv.slice(2);
...
for (const vp of [{ n: 'desktop', width: 1440, height: 900 }, { n: 'mobile', width: 390, height: 844 }, { n: 'small', width: 360, height: 800 }]) {
  ...
  if (vp.n !== 'small') {
    ...
    for (const h of hrefs) {
      ...
      if (q.get('source') !== `vino-demo-${slug}` || !q.get('company') || !/^https:\/\//.test(q.get('web') || '') || !q.get('demo')) fail(`${vp.n} contact link incomplete ${h}`);
    }
