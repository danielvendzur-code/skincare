(() => {
  ...
  if (originalTeaser) {
    ...
    teaser.innerHTML = `
      <button class="cx-teaser-open" type="button" aria-label="Otvoriť výber vína">
  ...
  /* Recommendation guard: a wine recommendation must first match the colour
     the customer chose; style and food only break the tie. */
  const answers = {};
  const recommendationScore = (product, selected) => {
    let total = 0;
    if (product.tags.includes(selected.skin)) total += 20;
    if (product.tags.includes(selected.goal)) total += 12;
    if (selected.texture !== 'any' && product.tags.includes(selected.texture)) total += 7;
    if (selected.routine === 'target' && product.tags.includes('target')) total += 4;
    return total;
  };
  /* Wine: the colour the customer asked for comes first; only a winery
     without that colour falls back to the chosen style. */
  const rankedProducts = (selected) => {
    const sameColour = brand.products.filter((product) => product.tags.includes(selected.skin));
    const primary = sameColour.length ? sameColour : brand.products.filter((product) => product.tags.includes(selected.goal));
  ...
  for (const skin of values.skin) for (const goal of values.goal) for (const routine of values.routine) for (const texture of values.texture) {
    ...
    if (hasColour && !winner.tags.includes(skin)) critical += 1;
  }
