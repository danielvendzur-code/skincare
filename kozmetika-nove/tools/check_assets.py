...
for slug in re.findall(r"^    ([a-z0-9]+): \{", config, re.M):
    ...
    paths = set(re.findall(r"/assets/cosmetics/[a-z0-9-]+\.(?:png|jpg)", block))
    paths |= {f'/assets/cosmetics/{slug}-{pid}.jpg' for pid in re.findall(r"photo:photo\('[a-z0-9]+','([a-z0-9]+)'\)", block)}
    paths.add(f'/assets/cosmetics/{slug}-logo.png')
