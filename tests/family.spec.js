...
for (const brand of brands) {
  test(`${brand}: storefront/owner, chat, advisor and result`, async ({ page }) => {
    ...
    if (brand === 'biofy') {
      await expect(page.locator('.biofy-storefront h1')).toBeVisible();
      await expect(page.locator('.biofy-products--face .biofy-product')).toHaveCount(3);
      await expect(page.locator('.biofy-products--hair .biofy-product')).toHaveCount(2);
      await expect(page.locator('.biofy-nav--desktop a')).toHaveCount(3);
      await expect(page.locator('body')).not.toContainText(/garantujeme rast|lieči|vylieči|dermatologicky testované/i);
    } else {
    ...
    if (brand === 'biofy') await page.getByRole('button', { name: /Opýtať sa v chate/i }).click();
    else await page.getByRole('button', { name: /Otvoriť Chat/i }).click();
    ...
    for (let step = 0; step < 4; step += 1) {
      const expectedChoices = brand === 'biofy' && step === 0 ? 2 : 4;
      await expect(page.locator('.choice-grid button')).toHaveCount(expectedChoices);
      await expect(page.locator('.choice-image img')).toHaveCount(expectedChoices);
      ...
      await page.locator('.choice-grid button').nth(step % expectedChoices).click();
...
test('brand themes are materially differentiated', async ({ page }) => {
  ...
  for (const brand of brands) {
    await page.goto(`/ukazka/${brand}`);
    if (brand === 'biofy') {
      signatures.push(await page.locator('.biofy-hero__visual').evaluate((node) => {
