...
for (const vp of [{ n: 'desktop', width: 1440, height: 900 }, { n: 'mobile', width: 390, height: 844 }, { n: 'small', width: 360, height: 800 }]) {
  ...
  if (vp.n !== 'small') {
    ...
    for (const h of hrefs) {
      ...
      if (q.get('source') !== `vlasy-demo-${slug}` || !q.get('company') || !/^https:\/\//.test(q.get('web') || '') || !q.get('demo')) fail(`${vp.n} contact link incomplete ${h}`);
    }
  ...
  // Hair segment: nothing the customer reads may still talk about skin.
  const skinWords = async (where) => { const t = await page.evaluate(() => document.querySelector('#cosmetics-root').innerText); const m = t.match(/[^.\n]*\bplet[ií][^.\n]*|[^.\n]*\bpleť[^.\n]*/i); if (m) fail(`${vp.n} ${where} still mentions skin: ${m[0].trim()}`); };
  await skinWords('chat');
