...
function Chat({ brand, startAdvisor }) {
  ...
  const send = async (text) => {
    ...
    const apiHistory = nextMessages.slice(-10).map((message) => ({
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ brand: brand.slug, messages: apiHistory }),
      });
      ...
      const fallback = typeof brand.fallback === 'function'
        ? brand.fallback(clean, apiHistory)
        : 'Momentálne sa neviem spojiť so službou. Skúste Výber starostlivosti.';
...
function Advisor({ brand }) {
  ...
  const choose = (option) => {
    ...
    const next = { ...answers, [question.key || String(step)]: option.value };
    setAnswers(next);
    ...
    window.setTimeout(() => {
      if (step === brand.questions.length - 1) {
        if (typeof brand.rankProducts === 'function') {
          setResult(brand.rankProducts(next));
        } else {
          const fallbackIndex = Math.abs(Object.values(next).join('').length) % brand.products.length;
          const product = brand.products[fallbackIndex];
          const alternative = brand.products.find((item) => item.url !== product.url) || brand.products[0];
          setResult({ product, alternative, reason: product.reason });
        }
      } else {
  ...
  const question = brand.questions[step];
  const answerKey = question.key || String(step);
  return <div className="advisor-view">
    <div className="advisor-top">
      ...
      <div className="progress"><b>{step + 1}/{brand.questions.length}</b><span>{brand.questions.map((_, item) => <i key={item} className={item <= step ? 'is-on' : ''} />)}</span></div>
    </div>
    {question.label && <span className="advisor-kicker">{question.label}</span>}
    <h2>{question.title}</h2>
    <p>{question.hint}</p>
    <div className="choice-grid">{question.options.map((option) => <button
      key={option.value}
      type="button"
      disabled={transitioning}
      aria-pressed={answers[answerKey] === option.value}
      className={answers[answerKey] === option.value ? 'is-selected' : ''}
...
function Result({ result, restart, back }) {
  const { product, alternative, reason, matchedTraits = [] } = result;
  return <div className="result-view">
    <div className="result-topline"><button className="result-back" type="button" onClick={back}><Icon name="back" /> Späť</button><span>Výsledok</span></div>
...
function Widget({ brand, open, setOpen, initialMode, onModeChange }) {
  ...
  return <>
    {!open && <div className="launcher-wrap">
      {teaserVisible && <div className="teaser">
        <button type="button" aria-label="Zavrieť pozvánku" onClick={() => setTeaserVisible(false)}>×</button>
        <b>{brand.teaserTitle}</b><span>{brand.teaser}</span>
      </div>}
      <button className="launcher" aria-label={`Otvoriť poradcu ${brand.name}`} aria-expanded={open} onClick={() => setOpen(true)}><Logo brand={brand} compact /></button>
    </div>}
    {open && <div className="overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <section className="widget" role="dialog" aria-modal="true" aria-label={`Poradca ${brand.name}`} tabIndex="-1" ref={panelRef}>
        <header className="widget__header"><Logo brand={brand} /><div><button type="button" aria-label="Začať odznova" onClick={() => setResetKey((value) => value + 1)}><Icon name="reset" /></button><button type="button" aria-label="Zavrieť" onClick={() => setOpen(false)}><Icon name="close" /></button></div></header>
...
function App() {
  ...
  const brand = normalizedSlug === 'mylo' ? myloConfig : brands[normalizedSlug];
  ...
  const page = brand.slug === 'mylo'
    ? <MyloStorefront brand={brand} openAdvisor={() => launch('advisor')} openChat={() => launch('chat')} />
