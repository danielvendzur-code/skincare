...
function Chat({ brand, startAdvisor, messages, setMessages }) {
  ...
  const send = async (text) => {
    ...
    const userMessage = { from: 'user', text: clean };
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ brand: brand.slug, messages: toApiMessages(nextMessages) }),
      });
  ...
  return <div className="chat-view">
    ...
    {initial ? <div className="quick-chips">{brand.chips.map((chip) => <button key={chip} onClick={() => send(chip)}>{chip}</button>)}</div> : null}
...
function Advisor({ brand }) {
  ...
  const questionCount = brand.questions.length;
  ...
  const choose = (option) => {
    ...
    window.setTimeout(() => {
      if (step === questionCount - 1) {
        const result = typeof brand.recommend === 'function'
          ? brand.recommend(next)
          : { product: brand.products[0], alternative: brand.products[1] ?? null, reason: brand.products[0]?.reason };
        setSelection(result);
      } else {
  ...
  return <div className="advisor-view">
    <div className="advisor-top">
      ...
      <div className="progress"><b>{step + 1}/{questionCount}</b><span>{brand.questions.map((_, index) => <i key={index} className={index <= step ? 'is-on' : ''} />)}</span></div>
    </div>
    <h2>{question.title}</h2>
    <p>{question.hint}</p>
    <div className="choice-grid">{question.options.map((option) => <button key={option.value} type="button" disabled={transitioning} aria-pressed={answers[step] === option.value} className={answers[step] === option.value ? 'is-selected' : ''} onClick={() => choose(option)}>
...
function Result({ product, alternative, reason, restart }) {
  return <div className="result-view">
    ...
    {alternative ? <div className="alternative"><span>Alternatíva v rovnakej produktovej roli</span><a href={alternative.url} target="_blank" rel="noreferrer">{alternative.name} · {alternative.price}</a></div> : <div className="alternative"><span>Alternatíva</span><small>V tejto produktovej roli je v overenom rozsahu dema jeden vhodný produkt.</small></div>}
...
function Widget({ brand, open, setOpen, initialMode, onModeChange }) {
  ...
  return <>
    {!open ? <div className="launcher-wrap">
      {teaserVisible ? <div className="teaser"><button aria-label="Zavrieť pozvánku" onClick={() => setTeaserVisible(false)}>×</button><b>{brand.teaserTitle}</b><span>{brand.teaser}</span></div> : null}
      <button className="launcher" aria-label={`Otvoriť poradcu ${brand.name}`} aria-expanded="false" onClick={() => setOpen(true)}><Logo brand={brand} compact /></button>
    </div> : null}
    {open ? <div className="overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <section className="widget" role="dialog" aria-modal="true" aria-label={`Poradca ${brand.name}`} tabIndex="-1" ref={panelRef}>
        <header className="widget__header"><Logo brand={brand} /><div><button aria-label="Začať odznova" onClick={() => { setResetKey((value) => value + 1); setChatMessages([{ from: 'bot', text: brand.welcome || `Dobrý deň. Čo dnes hľadáte v ponuke ${brand.name}?` }]); }}><Icon name="reset" /></button><button aria-label="Zavrieť" onClick={() => setOpen(false)}><Icon name="close" /></button></div></header>
        ...
        <div className="widget__body" key={`${resetKey}-${mode}`}>{mode === 'chat' ? <Chat brand={brand} startAdvisor={() => switchMode('advisor')} messages={chatMessages} setMessages={setChatMessages} /> : <Advisor brand={brand} />}</div>
      </section>
    </div> : null}
  </>;
}
...
function App() {
  ...
  const genericBrand = brands[brandOrder.includes(slug) ? slug : 'mylo'];
  const brand = slug === 'anemone' ? anemoneBrand : genericBrand;
  ...
  return <>
    {brand.slug === 'anemone'
      ? <AnemoneStorefront brand={brand} openAdvisor={() => launch('advisor')} openChat={() => launch('chat')} />
      : <OwnerPage brand={brand} openAdvisor={() => launch('advisor')} openChat={() => launch('chat')} />}
    <Widget brand={brand} open={open} setOpen={setOpen} initialMode={mode} onModeChange={setMode} />
  </>;
}
