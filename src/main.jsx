...
const Icon = ({ name }) => {
  const paths = {
    ...
    spark: <><path d="m12 3 1.4 4.1L17.5 8.5l-4.1 1.4L12 14l-1.4-4.1-4.1-1.4 4.1-1.4z" /><path d="M18.5 14l.8 2.2 2.2 .8-2.2 .8-.8 2.2-.8-2.2-2.2-.8 2.2-.8.8-2.2Z" /></>,
  };
...
function Chat({ brand, startAdvisor, messages, setMessages, busy, setBusy }) {
  ...
  const send = async (text) => {
    ...
    try {
      ...
      setMessages((items) => [...items, { from:'bot', text:brand.fallback(clean, withUser) }]);
    } finally {
      setBusy(false);
    }
  };
...
function Advisor({ brand }) {
  ...
  return <div className="advisor-view">
    ...
    <p>{question.hint}</p>
    <div className="choice-grid">{question.options.map((option) => <button key={option.value} data-value={option.value} type="button" disabled={transitioning} aria-pressed={answers[step] === option.value} className={answers[step] === option.value ? 'is-selected' : ''} onClick={() => choose(option)}>
  ...
  return <>
    ...
    {open ? <div className="overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <section className="widget" role="dialog" aria-modal="true" aria-label={`Poradca ${brand.name}`} tabIndex="-1" ref={panelRef}>
        ...
        <div className="widget__body" key={`${resetKey}-${mode}`}>{mode === 'chat'
...
function App() {
  ...
  const brand = normalized === 'two' ? twoBrand : brands[normalized];
